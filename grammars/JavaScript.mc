package se.rwth.javascript.lang;
...
\f 	form feed
  */               
  fragment token DoubleStringChar = ('\\' EscapeSequence
                                     | LineContinuation 
                                     | ~('"' | '\\' | '\n' | '\r' | '\u2028' | '\u2029') );

  fragment token SingleStringChar = ('\\' EscapeSequence
                                     | LineContinuation 
                                     | ~('\'' | '\\' | '\n' | '\r' | '\u2028' | '\u2029') );
                                                                          
  fragment token LineContinuation = '\\' LineTerminatorSequence;
  ...
  fragment token NonEscapeCharacter = 
  ...
  fragment token DecimalIntegerLiteral = ('0' | '1'..'9' (DecimalDigits)?);
  fragment token DecimalDigits = (DecimalDigit)+;
  fragment token DecimalDigit = ('0'..'9');
  fragment token ExponentPart = ('e'|'E') ('+'|'-')? DecimalDigits;

  
  HexIntegerLiteral implements NumericLiteral = Hex;
  token Hex = '0' ('x'|'X') (HexDigit)+;
  fragment token HexDigit = ('0'..'9'|'a'..'f'|'A'..'F');
  fragment token HexEscapeSequence = 'x' HexDigit HexDigit;
  
  fragment token UnicodeEscapeSequence = 'u' HexDigit HexDigit HexDigit HexDigit;
...
System.out.println(allowDivPunctuator());
                          ...
                          ( ("/" REBody "/")=> "/" REBody "/" (Name)? |
                           ...
                           ("/") => "/" {_ttype = SLASH;});
*/
  fragment token REBody = REFirstChar (REChar)*;
  fragment token REFirstChar = (REClass | ('\\' RENonTerminator) | RENoStarLBracket );
  fragment token REChar = (REClass | ('\\' RENonTerminator) | RENoLBracket );
  fragment token RENoStarLBracket = ( ~('*' | '[' | '\n' | '\r' | '/' | '\\' ) ) ;
  fragment token RENonTerminator = (~('\n' | '\r' ) ) ;
  fragment token RENoLBracket = ( ~('[' | '\n' | '\r' | '/' | '\\' ) ) ;
  fragment token RENoRBracket = ( ~(']' | '\n' | '\r' | '\\' ) ) ;
  fragment token REClass = '[' (RENoRBracket | ('\\' RENonTerminator))* ']';
  ...
  */
  ...
  concept antlr {
    lexer java {
      ...
      @Override
      fragment Token makeToken(int t) {
        setLastToken(t);
